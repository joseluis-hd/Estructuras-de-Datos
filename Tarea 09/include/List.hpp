...
class List
{
    ...
    Node<T>* findData(T, int comp(const T&, const T&));
...
template <class T>
Node<T>* List<T>::findData(T e, int comp(const T&, const T&))
{
    Node<T>* aux(anchor);
...
string List<T>::toString() const
{
    ...
    if (isEmpty())
    {
        throw Exception("Insuficiencia de datos, toString");
    }

    while (aux != nullptr)
    {
        writtenList << aux->getData().toString();
        aux = aux->getNext();
    }
